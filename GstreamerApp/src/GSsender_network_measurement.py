...
Equivalent to:
gst-launch-1.0 -e -v v4l2src device=/dev/video4 do-timestamp=true ! \
    ...
    udpsink host=10.147.20.186 port=5004 sync=false async=false
"""
...
class GStreamerSender:
    def __init__(self, device="/dev/video4", host="10.147.20.186", port=5004,
                 width=1920, height=1080, framerate=30, bitrate=6000,
                 metrics_log_dir="metrics_logs"):
        """
        Initialize the GStreamer sender pipeline.
        
        Args:
            device: V4L2 device path (e.g., /dev/video4)
            ...
            metrics_log_dir: Directory name for saving metrics files
        """
        self.device = device
        self.host = host
        self.port = port
        ...
        self.metrics_log_dir_name = metrics_log_dir
        ...
        # Per-minute tracking for 24-hour logging
        self._minute_rtt_values = []
        self._minute_rtt_lock = threading.Lock()
        self._minute_packets_sent = 0
        self._minute_packets_received = 0
        self._minute_packet_lock = threading.Lock()
        
        # 24-hour run configuration
        self._run_duration_hours = 24
        self._start_time = None
        self._metrics_dir = None
        self._metrics_timer_running = False
    ...
    def _on_rtp_out(self, pad, info):
        ...
        try:
            ...
            # Increment per-minute packets sent counter
            with self._minute_packet_lock:
                self._minute_packets_sent += 1
        finally:
            rtp.unmap()
                    ...
                    if len(data) == 2:
                        ...
                        with self.t1_lock:
                            if seq in self.t1:
                                ...
                                # Store RTT value for per-minute statistics
                                with self._minute_rtt_lock:
                                    self._minute_rtt_values.append(rtt_ms)
                                
                                # Increment per-minute packets received counter
                                with self._minute_packet_lock:
                                    self._minute_packets_received += 1
    ...
    def _setup_metrics_directory(self):
        """Create directory for saving metrics files."""
        script_dir = os.path.dirname(os.path.abspath(__file__))
        self._metrics_dir = os.path.join(script_dir, self.metrics_log_dir_name)
        os.makedirs(self._metrics_dir, exist_ok=True)
        print(f"Metrics will be saved to: {self._metrics_dir}")
    
    def _save_minute_metrics(self):
        """Save current minute's metrics to a file and reset counters."""
        timestamp = datetime.now()
        filename = timestamp.strftime("%Y%m%d_%H%M%S") + ".json"
        filepath = os.path.join(self._metrics_dir, filename)
        
        # Collect per-minute RTT statistics
        with self._minute_rtt_lock:
            minute_rtt_values = self._minute_rtt_values.copy()
            self._minute_rtt_values = []
        
        # Collect per-minute packet statistics
        with self._minute_packet_lock:
            minute_sent = self._minute_packets_sent
            minute_received = self._minute_packets_received
            self._minute_packets_sent = 0
            self._minute_packets_received = 0
        
        # Calculate RTT statistics for this minute
        if len(minute_rtt_values) > 0:
            n = len(minute_rtt_values)
            mean_rtt = sum(minute_rtt_values) / n
            if n > 1:
                variance = sum((x - mean_rtt) ** 2 for x in minute_rtt_values) / (n - 1)
                std_rtt = variance ** 0.5
            else:
                std_rtt = 0.0
            min_rtt = min(minute_rtt_values)
            max_rtt = max(minute_rtt_values)
        else:
            n = 0
            mean_rtt = None
            std_rtt = None
        ...
        # Calculate packet loss rate for this minute
        if minute_sent > 0:
            packet_loss_rate = (minute_sent - minute_received) / minute_sent * 100
        else:
            packet_loss_rate = None
        
        # Calculate elapsed time
        elapsed_seconds = (timestamp - datetime.fromtimestamp(self._start_time)).total_seconds()
        ...
        metrics = {
            ...
            "rtt": {
                "samples": n,
                "mean_ms": round(mean_rtt, 3) if mean_rtt is not None else None,
                "std_ms": round(std_rtt, 3) if std_rtt is not None else None,
                "min_ms": round(min_rtt, 3) if min_rtt is not None else None,
                "max_ms": round(max_rtt, 3) if max_rtt is not None else None
            },
            "packet_loss": {
                "packets_sent": minute_sent,
                "packets_received": minute_received,
                "loss_rate_percent": round(packet_loss_rate, 3) if packet_loss_rate is not None else None
            }
        }
        ...
        try:
            ...
            print(f"[{timestamp.strftime('%H:%M:%S')}] Metrics saved: RTT={mean_rtt:.2f}ms (n={n}), Loss={packet_loss_rate:.2f}%" if mean_rtt and packet_loss_rate else f"[{timestamp.strftime('%H:%M:%S')}] Metrics saved (no data)")
        except Exception as e:
            print(f"Error saving metrics: {e}")
    
    def _start_metrics_timer(self):
        """Start the periodic timer to save metrics every minute for 24 hours."""
        self._metrics_timer_running = True
        self._start_time = time.time()
        total_duration_seconds = self._run_duration_hours * 3600  # 24 hours in seconds
        
        def timer_thread():
            next_save_time = time.time() + 60  # First save after 1 minute
            
            while self._metrics_timer_running:
                current_time = time.time()
                elapsed = current_time - self._start_time
                
                # Check if 24 hours have passed
                if elapsed >= total_duration_seconds:
                    print(f"\n24-hour measurement complete. Total elapsed: {elapsed/3600:.2f} hours")
                    self.stop()
                    break
                
                # Check if it's time to save metrics (every minute)
                if current_time >= next_save_time:
                    self._save_minute_metrics()
                    next_save_time = current_time + 60
                
                # Sleep for a short interval to avoid busy-waiting
                time.sleep(0.5)
        
        self._metrics_timer_thread = threading.Thread(target=timer_thread, daemon=True)
        self._metrics_timer_thread.start()
        print(f"Started 24-hour metrics collection. Saving metrics every minute.")
    
    def _stop_metrics_timer(self):
        """Stop the metrics timer."""
        self._metrics_timer_running = False
        if self._metrics_timer_thread:
            self._metrics_timer_thread = None
    ...
    def start(self):
        ...
        # Setup metrics directory and start periodic saving
...
def main():
    ...
    sender = GStreamerSender(
        ...
        metrics_log_dir=sender_config.get("metrics_log_dir", "metrics_logs")
    )
